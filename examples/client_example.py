...
def example_1_after_login():
    ...
    users = auth.client.get_list(
        "User", fields=["name", "email", "full_name"], filters={"enabled": 1}, limit_page_length=10
    )
    print(f"Found {len(users)} users")

    # Insert a document
    new_todo = auth.client.insert({"doctype": "ToDo", "description": "Test todo from auth bridge"})
    print(f"Created ToDo: {new_todo.get('name')}")

    # Update a document
    auth.client.update({"doctype": "ToDo", "name": new_todo.get("name"), "status": "Closed"})
...
def example_5_session_client():
    ...
    # Use it
    roles = client.get_list("Has Role", fields=["role"], filters={"parent": session.user.email})
...
def example_6_full_workflow():
    ...
    new_doc = auth.client.insert(
        {"doctype": "Note", "title": "Test Note", "content": "Created via frappe-auth-bridge"}
    )
    print(f"   Created Note: {new_doc.get('name')}")

    # Make an API call
    print("\n5. Making API call...")
    result = auth.client.get_api(
        "frappe.client.get_list", {"doctype": "User", "fields": '["name"]', "limit_page_length": 5}
    )
