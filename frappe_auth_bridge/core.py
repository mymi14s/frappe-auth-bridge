...
from frappe_auth_bridge.models import User, Session, Permission, TenantConfig
from frappe_auth_bridge.session import SessionStore, MemorySessionStore
from frappe_auth_bridge.security import EncryptionManager, TokenBucketRateLimiter, generate_secure_token
...
class FrappeAuthBridge:
        ...
        try:
            ...
            if self.audit_logger:
                self.audit_logger.login_success(
                    user.email,
                    metadata={"tenant_id": tenant_id, "roles_count": len(roles)}
                )
            ...
            if self.audit_logger:
                self.audit_logger.login_failure(
                    username,
                    str(e),
    ...
    def authenticate_api_key(
        ...
        try:
            ...
            if self.audit_logger:
                self.audit_logger.login_success(
                    user.email,
                    metadata={"auth_method": "api_key", "tenant_id": tenant_id}
                )
    ...
    def get_user_permissions(
        self, user_email: str, use_cache: bool = True
    ) -> List[Permission]:
        """
        ...
        if self._client is None:
            ...
            if 'username' in self._client_credentials:
                ...
                self._client.authenticate(
                    self._client_credentials['api_key'],
                    self._client_credentials['api_secret']
                )
        
        return self._client
    
    def set_client_credentials(self, username: str, password: str, tenant_id: Optional[str] = None) -> None:
        """
    ...
    def set_client_api_key(self, api_key: str, api_secret: str, tenant_id: Optional[str] = None) -> None:
        """
    ...
    def _fetch_user_roles(self, client: FrappeClient, username: str) -> List[str]:
        ...
        try:
            roles_data = client.get_list(
                "Has Role",
                fields=["role"],
                filters={"parent": username, "parenttype": "User"}
            )
            return [role['role'] for role in roles_data]
        except Exception:
            return []
    
    def _fetch_user_permissions(
        self, client: FrappeClient, username: str
    ) -> List[Permission]:
