...
from frappe_auth_bridge import (AuthenticationError, FrappeAuthBridge,
                                SessionExpiredError)
...
def test_session_expiry():
    ...
    user = User(email="test@example.com", name="test", roles=["User"])
...
def test_multi_tenant_configuration():
    ...
    tenant_config = TenantConfig(tenant_id="company_a", frappe_url="https://company-a.erpnext.com")
...
def test_permission_validation(auth_bridge):
    ...
    auth_bridge.permission_cache.set_user_permissions(
        user_email, [Permission(doctype="User", read=True, write=False)]
    )
